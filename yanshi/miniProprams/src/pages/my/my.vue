  ...
  <div class="myWrap">
    ...
    <div class="itemsWrap">
      ...
      <div class="item mys_03" @click="goList(3)">
        ...
        <div class="rt">
          <span v-if="isLogin && czCount">{{czCount}}个</span>
          <i class="icon iconfont icon-jiantou"></i>
        </div>
        <span class="divssion"></span>
      </div>
    ...
    <button v-if="!isLogin" type="primary" open-type="getUserInfo" @getuserinfo="getUserInfo" class="btn_auth">小程序授权</button>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async _init() {
      const sessionRes = await this.API.interfaces.checkLogin();
