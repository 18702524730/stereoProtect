<template>
  <div class="list_info customs_list_info">
    ...
    <scroll-view :scroll-y="true" class="orderWrap" v-if="dataLists && dataLists.length && loaded" upper-threshold @bindscrolltoupper="scrollT" @bindscroll="scrollT" @bindscrolltolower="addLists" :scroll-top="indexTop">
      <div class="orderItem" v-for="(item, index) in dataLists" :key="index" @click="goDetail(item.orderSn)">
        ...
        <div class="item"><p class="des">知识产权类型：</p><p class="content">{{item.intellectualPropertyTypeText}}</p></div>
        <div class="item"><p class="des">服务类型：</p><p class="content">{{item.ciqServiceTypeText}}</p></div>
...
<script>
  ...
  export default {
    ...
    methods: {
      goDetail(orderSn) {
        this.$router.push(`/pages/my/customsDetail?orderSn=${orderSn}`);
      },
      async search(){//初始化
        let self = this;
        let data = {
          // orderState:this.orderState,
          stereoProtectType: 4, //1,电商侵权投诉；2,电商维权申诉,3;质量抽检鉴定;4,海关知产保护
        ...
        try {
          ...
          resp.elements.forEach((item, index) => {
            ...
            item.intellectualPropertyTypeText = Filters.filters.intellectualPropertyType(item.intellectualPropertyType);
            item.ciqServiceTypeText = Filters.filters.ciqServiceType(item.ciqServiceType);
