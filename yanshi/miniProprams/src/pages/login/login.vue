    ...
    <div v-if="!show" class="login_wrap">
      <div class="login_form">
        ...
        <p class='line row-between'>
          ...
          <input type="number" class="input" @blur="codeBlur" @focus="codeFocusState = true" maxlength="6" v-model="token">
          <span class="link" :class="{'disabled': !preventCode, 'isOver': isOver}"  @click.stop="getCode">{{ codeText }}</span>
        </p>
        <p class="tips">温馨提示：未注册快公证的手机号，登录时将自动注册，且代表已同意<span class="link" @click="agreementShow = true">《快公证用户协议》</span></p>
...
export default {
  ...
  methods: {
    ...
    initFn () {
      ...
      console.log('auth', this.$route.query.showAuthorize, this.show)
      if(this.show){
...
<style lang="scss">
  .login_page {
    ...
    .authorize_wrap {
      ...
      .btn_auth {width: 600px; height: 90px; line-height: 90px; padding: 0; border-radius: 6px; background:linear-gradient(90deg,rgba(246,71,68,1) 0%,rgba(246,111,68,1) 100%); color: #fff; font-size: 32px; text-align: center; margin: 60px auto 0 ;display: block; }
    }
    .login_wrap {
      .logo_box {
      ...
      .login_form {margin-top: 90px;
        ...
        .line {
          ...
          .link {color: #999; font-size: 28px; height: 56px;line-height: 56px;padding: 0; border: 0;}
          .disabled {color: #999}
          .isOver{color: #F64744;}
        ...
        .submit_btn {width: 100%; height:90px; line-height: 90px; padding: 0; border-radius: 6px; background:linear-gradient(90deg,rgba(246,71,68,1) 0%,rgba(246,111,68,1) 100%); color: #fff; font-size: 36px; text-align: center; margin-top: 60px;border: none;}
