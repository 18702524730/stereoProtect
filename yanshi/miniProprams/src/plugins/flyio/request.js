...
// session过期页面重载，重新执行onLoad,onShow 钩子函数
const pageReload = () => {
  let pagesArr = getCurrentPages();
  if (pagesArr.length != 0) {
    //刷新当前页面的数据
    pagesArr[pagesArr.length - 1].onLoad()
    pagesArr[pagesArr.length - 1].onShow()
  }
};
...
const rMethod = (url = '', data = {}, flyConfig = {}, defaultTipConfig = {}) => {
  ...
  loadingTimer = setTimeout(() => {
    tipConfig.isLoading && tipConfig.isLoading !== 'none' && Config.loading.loadingShow()
  ...
  return flyio.then(res => {
    ...
    tipConfig.isLoading && tipConfig.isLoading !== 'none' && Config.loading.loadingHide() // 当promise全部加载完成则隐藏loading
    ...
    tipConfig.isLoading && tipConfig.isLoading !== 'none' && Config.loading.loadingHide()
    !tipConfig.isLoading && wx.hideNavigationBarLoading()
    if (err && err.response && err.response.data.status === 401) {
