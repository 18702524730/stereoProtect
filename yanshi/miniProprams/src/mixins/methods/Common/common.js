const { cartUrl, indexPath } = CONFIG;
import API from '@/api'
export default {
  ...
  // 微信支付
  wechatPay (params, success, fail) {
    let self = this;
    let paymentBody = {};
    paymentBody.orderSn = params.orderSn;
    paymentBody.paymentSn = params.paymentSn;
    paymentBody.paymentType = 1; //支付方式1:公众号支付，2：小程序支付，不传默认1
    paymentBody.openid = params.openid;
    let token = self.$storage.get('key');
    let sessionId = self.$storage.get('sessionId');
    let url = cartUrl + '/payweb/wxpayment.htm';
    wx.request({
      url: url,
      method: 'POST',
      data: paymentBody,
