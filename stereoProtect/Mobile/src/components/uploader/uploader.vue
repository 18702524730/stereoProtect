<template>
    ...
    <div class="file-uploader" :class="{'canPreview': canPreview}" @click="addFile">
...
<script>
    ...
    props:{
      ...
      // action
    	postAction:{
    		type: String,
    		default: rootUrl + '/file/upload.htm'
    	},
      // name
    	name: {
    		type: String,
      ...
      inputFile(newFile, oldFile) {
        ...
        if (newFile && !oldFile) {
            ...
            this.$refs.etc.update(newFile, {blob: URL.createObjectURL(newFile.file)})
				...
				if (newFile && oldFile) {
					...
					if (this.$refs.etc.uploaded) {
						...
						if (err == 'extension') {
							this.startShow = false;
							this.$toast('上传文件的格式不支持');
							return;
						}
						if (err == 'size') {
							this.startShow = false;
							this.$toast('上传文件大小不能超过'+ (this.size < 1 ? this.size*1000 + 'KB' : this.size + 'M'));
							return;
						}
          ...
          if (data.code) {
			...
			transformPdf(url){
				var self = this;
				self.$http.get(rootUrl+'/api/preview?pdf_url='+url)
				.then((response) => {
...
<style lang='less'>
	.upload_wrap{
		...
		.file-uploads{opacity: 0;-ms-filter: 'progid: DXImageTransform.Microsoft.Alpha(Opacity=0)'; position: absolute;left: 0;top: 0;width: 2rem;height: 2rem;cursor: pointer;}
    .file-uploader{
	  float:left;width:2rem;height: 2rem;background: #ECEEF2 url(~assets/img/home/phono.png) center center no-repeat;border-radius: 2px;cursor: pointer;text-align: center;
