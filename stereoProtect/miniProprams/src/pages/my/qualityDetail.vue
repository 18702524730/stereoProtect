<template>
  <div class="orderDetail qualityDetail">
    ...
    <div class="box detail">
      <h3>质量抽检鉴定详情信息</h3>
      <div class="item"><p class="des">抽检平台</p><p class="content">{{orderDetail.platform}}</p></div>
      <div class="item"><p class="des">商品网址</p><p class="content">{{orderDetail.website}}</p></div>
      <div class="item"><p class="des">抽检理由描述</p><p class="content">{{orderDetail.content}}</p></div>
      <div class="btItem"> <!-- v-if="orderDetail.infringeImgArr" -->
        <p>凭证</p>
        <div class="imgWrap" @click="preview">
          <img v-for="(item, index) in orderDetail.voucherUrlsArr" :key="index" :data-index="index" mode="aspectFill" :src="item" alt="">
