...
export default {
  ...
  onShow () {
    if (this.authInfo && this.authInfo.apply_id) {
      console.log('authInfo', this.authInfo)
      this.dataList.forEach(item => {
        if (item.apply_id == this.authInfo.apply_id) {
          item.is_checked = this.authInfo.is_checked;
        }
      })
    }
  },
  ...
  methods: {
    ...
    async subject (type) {
      try{
        ...
        let isLogin = await err;
        // isLogin && this.pageReload();
