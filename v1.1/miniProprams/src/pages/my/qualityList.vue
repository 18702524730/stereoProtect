<template>
  <div class="list_info quality_list_info">
    ...
    <scroll-view :scroll-y="true" class="orderWrap" v-if="dataLists && dataLists.length && loaded" upper-threshold @bindscrolltoupper="scrollT" @bindscroll="scrollT" @bindscrolltolower="addLists" :scroll-top="indexTop">
      <div class="orderItem" v-for="(item, index) in dataLists" :key="index" @click="goDetail(item.orderSn)">
        ...
        <div class="item"><p class="des">抽检平台：</p><p class="content">{{item.platform}}</p></div>
        <div class="item"><p class="des">商品网址：</p><p class="content">{{item.website}}</p></div>
...
<script>
  ...
  export default {
    ...
    methods: {
      goDetail(orderSn) {
        this.$router.push(`/pages/my/qualityDetail?orderSn=${orderSn}`);
      },
      async search(){//初始化
        let self = this;
        let data = {
          // orderState:this.orderState,
          stereoProtectType: 3, //1,电商侵权投诉；2,电商维权申诉,3;质量抽检鉴定;4,海关知产保护
