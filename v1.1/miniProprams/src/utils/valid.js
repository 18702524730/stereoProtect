...
const strategys = {
  ...
  // 网址校验  ^((https|http|ftp|rtsp|mms)?:\/\/)
  websiteFormat: function (value, errorMsg) {
  ...
  // 专利申请号
  patentSnFormat: function (value, errorMsg) {
    let reg = /^[a-zA-Z0-9\.]{13,16}$/;
    if (!reg.test(value)) {
      return errorMsg;
    }
  },
  // 银行卡 
  bankCarkFormat: function (value, errorMsg) {
    let reg = /^[1-9]{1}\d{10,18}$/;
    if (!reg.test(value)) {
      return errorMsg;
    }
  },
